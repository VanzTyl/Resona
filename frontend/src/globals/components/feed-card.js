...
function createFeedCard(cardData, onReaction) {
    ...
    VALID_EMOJIS.forEach(function (emoji) {
        const emojiBtn = document.createElement('button');
        emojiBtn.className = 'feed-card__emoji-btn';
        emojiBtn.textContent = emoji;
        emojiBtn.setAttribute('aria-label', 'React with ' + emoji);
        emojiBtn.setAttribute('title', 'React with ' + emoji);

        emojiBtn.addEventListener('click', function () {
            if (typeof onReaction === 'function') {
                onReaction(cardData.id, emoji);
            }
        });
