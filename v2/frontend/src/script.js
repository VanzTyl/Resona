...
// v1.1: createIcon, initIcons, setTheme, startNowPlayingPolling also available
...
const RESONA_ROUTES = {
    ...
    '/friends': {
        controller: 'friends',
        requiresAuth: true,
        title: 'Resona — Find Friends',
    },
    '/messages': {
        controller: 'messages',
        requiresAuth: true,
        title: 'Resona — Messages',
    },
    '/dashboard': {
    ...
    '/profile': {
        controller: 'profile',
        requiresAuth: true,
        title: 'Resona — Profile',
    },
    // v1.1: Onboarding route
    '/onboarding': {
...
const RESONA_NAV_ITEMS = [
    { route: '/feed', label: 'Feed', icon: 'music' },
...
async function handleRouteChange() {
    ...
    if (routeConfig.requiresAuth) {
        ...
        // v1.1: Onboarding guard — skip check for onboarding and callback pages
        if (hash !== '/onboarding' && hash !== '/callback') {
            try {
                ...
                // If onboarding check fails, allow access (non-blocking)
            }
        }
    }
    ...
    // v2.1: Render desktop sidebar on authenticated pages.
    renderDesktopSidebar(hash);

    // v2.1: Listen for viewport resize to show/hide sidebar.
    setupSidebarResizeListener(hash);
...
function renderBottomNav(activeRoute) {
    ...
    // Only show nav on main app pages, not login, callback, or onboarding.
    const hideNavRoutes = ['/login', '/callback', '/onboarding'];
    ...
    RESONA_NAV_ITEMS.forEach(function (item) {
        ...
        // v1.1: Use Lucide icons instead of text emoji
        const iconContainer = document.createElement('span');
        iconContainer.className = 'bottom-nav__icon';

        if (typeof createIcon === 'function') {
            const iconEl = createIcon(item.icon, { size: 24 });
...
/**
 * Set up a resize listener to toggle sidebar visibility on viewport change.
 *
 * @param {string} activeRoute - Currently active route.
 * @returns {void}
 */
 ...
 * v2.1: Creates sidebar with brand, nav links, user info, theme toggle, and logout.
 *
 * @param {string} activeRoute - Currently active route.
 * @returns {void}
 */
function renderDesktopSidebar(activeRoute) {
    ...
    // Only show sidebar on authenticated pages at desktop width.
    ...
    // Try to load user profile for avatar and name.
    ...
    // Actions row (theme toggle + logout).
 ...
 * Non-blocking — sidebar shows loading state until profile loads.
 *
...
async function loadSidebarUserProfile(avatarEl, userInfoEl) {
    try {
        ...
        // Non-blocking — sidebar remains functional with default avatar.
    }
}
